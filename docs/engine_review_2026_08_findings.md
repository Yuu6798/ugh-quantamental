...
**判定結果 (2026-08-31 追記 — FX-ESTAR-LAG 実施済み、詳細は
`docs/analysis/estar_lag_2026_08.md`)**: **上記仮説は棄却**。`spot_vs_sma20` の
ablation は持続的転換日を全 6 セル (α/β/δ × 参照値 2 種) で **0 営業日**しか
動かさなかった — `fundamental_score` の ~19 営業日連続マイナス (うち clamp 飽和
10 日) は転換タイミングと相関するだけで律速ではない (同時転換 ≠ 因果、の実例)。
一貫した律速は **`momentum_5d`** (ablation で全セル −2〜−10 営業日早まる)。なお
raw e_star 符号の転換日 (β 8/5 / δ 8/11 / γ 8/18 / α 8/19) は emitted 方向ベースの
年表 (β 8/11 / δ 8/19 / α・γ 8/21) より epsilon dead-zone の分だけ早い — 順序は
一致しており矛盾ではない (方法論差)。9 月の follow-up 候補: `momentum_5d` と
variant 方向 weight の相互作用 (観察であり因果未証明、の扱いを維持)。
