...
- [ ] **in-place 再定義の禁止 (semantics 混在防止)**。Option A を採る場合でも、
      ...
      汚染される。よって **列 rename / 新メトリクス追加 (実質 Option B) を必須**と
      する。同一列 `state_proxy_hit` の in-place 再定義は禁止。
      ※ schema/protocol version bump + filtering 案は**採らない**: `schema_version`/
      `protocol_version` は 3 箇所で `v1` 既定 (`automation_models.py:23-24` /
      `fx-daily-protocol.yml:40-41` / `scripts/run_fx_daily_protocol.py:59-60`)
      で、それらは本 brief の scope 外 (engine_version と異なり eval-metric 変更に
      巻き込むべきでない)。新列なら version filtering 自体が不要
