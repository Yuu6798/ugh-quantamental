...
| Step | Action |
|---|---|
| 1 | Bump **both** `FX_THEORY_VERSION` and `FX_ENGINE_VERSION` from `v1` to `v2` in **all three** locations: (a) `.github/workflows/fx-daily-protocol.yml` and `.github/workflows/fx-analysis-pipeline.yml` (env-var `\|\|` defaults); (b) the manual-run env-var defaults in `scripts/run_fx_daily_protocol.py` (lines 57–58); (c) the Pydantic field defaults on `FxDailyAutomationConfig` in `src/ugh_quantamental/fx_protocol/automation_models.py` (lines 21–22). All three are independent default sources and a v1 default surviving in any one of them would falsely label v2-engine forecasts as `engine_version=v1` (or `theory_version=v1`) — breaking audit, replay, and the §7.5 stratification this migration relies on. Direct automation callers and tests that construct `FxDailyAutomationConfig()` without overriding the version fields go through (c). Both bumps (theory + engine) are required because v2 changes the engine math (`compute_e_raw`, `fire_probability`); records persist both fields in `forecast_record.{theory_version,engine_version}` for stratification. |
