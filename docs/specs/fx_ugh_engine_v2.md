...
| 4c | **Migrate hard-coded batch-size `4` checks to the new daily batch size of 7** (3 baselines + 4 v2 variants). The current code asserts `len(forecasts) == 4` (or `!= 4`) in seven places across three files; if these stay at 4, the first v2 batch with 7 forecasts will be treated as partial/incomplete on retry, blocking outcome evaluation and breaking the daily cycle. Sites verified via grep (May 2026): `forecasting.py:250` (`run_daily_forecast_workflow` partial-batch error path), `forecasting.py:252` (its error message text), `outcomes.py:328` (`run_daily_outcome_evaluation_workflow` reject), `outcomes.py:331` (error message), `outcomes.py:354` (existing-evaluations existence check), `outcomes.py:361` (error message), `automation.py:296` (existing-forecast-batch ready check in `run_fx_daily_automation`), `automation.py:324` (prior-batch ready check). Recommended fix: replace the literal `4` with a module-level constant `EXPECTED_DAILY_BATCH_SIZE = len(_ACTIVE_STRATEGY_KINDS)` derived from a single source-of-truth tuple of v2-active strategies; this also lets future variant additions/removals propagate automatically. Error message strings need the same numeric replacement. |
