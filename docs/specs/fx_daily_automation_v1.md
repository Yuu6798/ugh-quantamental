...
3. It is **not yet fully recovered**: either no `OutcomeRecord` exists for `(pair, w.window_start_jst, w.window_end_jst, schema_version)` (or one exists but its evaluation batch is incomplete) — in which case the evaluation workflow runs — **or** the DB evaluation is already complete but its history CSVs are not (see § CSV export below) — in which case the evaluation workflow is skipped and only publication is retried, so a window is never re-evaluated once its `OutcomeRecord` exists. A window whose DB evaluation *and* history CSVs are both already complete is a silent idempotent no-op (not re-reported in `catchup_windows`).
...
A catch-up recovery writes its outcome/evaluation CSVs into `history/{date_str}/{forecast_batch_id}/`, via `csv_exports.publish_csv_to_history_only` — a history-only counterpart to `publish_csv_to_layout` that never touches `latest/`. This keeps `latest/` pointing at the **current** day's batch even after a multi-window catch-up recovery, whereas `publish_csv_to_layout` (used for the current day's own window in step 8) intentionally updates both `history/` and `latest/`.

`forecast_batch_id` is the recovered window's own originating batch
(`make_forecast_batch_id(pair, w.window_start_jst, protocol_version)`) —
the same value in both the normal and catch-up paths. **`date_str`,
however, is the window's END date (`w.window_end_jst`), never its start
date.** The start-date directory is already owned by that batch for its
own `forecast.csv` (published the day the forecast was generated) and,
whenever that start date's own run evaluated a window normally via step 6,
for *that* window's outcome/evaluation too — filed there under the
**next** day's own new forecast-batch id, which happens to be numerically
...
*different* (start-date) directory that may not exist — e.g. the original
day's own publish step itself failed after its evaluation flushed —
catch-up additionally re-exports the persisted batch's forecast rows (via
`export_daily_forecast_csv`, from the DB records already loaded for the
batch-completeness check) and passes them as `publish_csv_to_history_only`'s
optional `forecast_path` argument, so the end-date directory is
...
one); `collect_evaluated_forecast_rows` dedupes by `forecast_id` so this
never double-counts a forecast or its evaluation in a rebuild.
