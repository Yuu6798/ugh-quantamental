...
> range の更新が必要。
>
> **NOTE (2026-06-01, P2B 実装後の訂正)**: 「evidence層 softmax T=0.5
> **だけで** dominant_state の knife-edge が解消する」という当初仮説は
> **不十分**だった。`run_state_engine` の決定経路は
> `evidence_raw → softmax(T=0.5) → blend_with_prior(prior 0.55/evid 0.45)
> → _normalize_simple(線形) → build_phi` であり、evidence層を尖らせても
> (a) prior_weight 0.55 の blend と (b) 最終段の線形正規化で washout され、
> 実際に dominant_state を読む `updated_probabilities` 層には伝播しない
> (evidence層 60/60 vs 最終層 14/60)。第一レバー(prior/evidence 比) では
> fire と winner-margin が**原理的トレードオフ**(fire は prior 保温に依存、
> margin は evidence 鋭さを要求) で両立不可と replay で確定。
> **解は二段温度**: evidence層 `softmax_temperature=0.5` を保ちつつ
> `_normalize_simple` を softmax 化し独立した
> `final_softmax_temperature=0.12` で最終層を尖らせる。これで fire
> (16/60、温度不変) と winner-margin (41/60=68%) が分離・両立した
> (PR #109)。詳細は `docs/specs/ugh_state_engine_v1.md` の二段温度記述。
