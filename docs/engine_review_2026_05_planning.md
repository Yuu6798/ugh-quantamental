...
| メトリクス | Phase A 期 (現状: range variant 共有) | Phase B 完了後 (range per-variant) | 不変 |
|---|---|---|---|
...
| `range_hit_count` | **per-batch** | **per-variant に戻す** | Phase B で range も変種依存になる |
| `state_proxy_hit_count` | per-batch | per-batch (要再評価) | state は依然 variant 非依存 |
| `dominant_state` 分布 | per-batch | per-batch (要再評価) | 同上 |

> **重要**: `range_hit_count` を per-batch に集約するのは **Phase A 期の暫定
...
>
> `state_proxy_hit` / `dominant_state` は Phase B 後も variant 非依存だが、
> §4 の state classifier sharpening で挙動が変わるため Phase 2 完了後に
> 集計方針を再評価する。

これにより:
- Phase A 完了時点で `weekly_strategy_metrics.csv` の `range_hit_count` 列は
  variant 別では
