...
- Phase A 完了時点で `weekly_strategy_metrics.csv` には UGH variant 4 行
  ...
  - 具体的な実装 (`weekly_reports_v2.build_strategy_metrics`):
    1. variant 行構築時に `range_hit_count` / `range_hit_rate` を `""` で
       上書き
    2. 全 v2 variant 行を入力に `_compute_metrics_for_rows` を 1 回呼び、
       結果のうち `range_hit_count` / `range_hit_rate` のみを抽出
...
- **slice metrics (`weekly_slice_metrics.csv`) も Phase A スコープに含める**。
  ...
  - variant 行では `range_hit_count` / `range_hit_rate` を `""` 化、
    ensemble 行のみが `(dim, label)` 配下の全 v2 variant rows を入力に
    した集計値を持つ。
...
- monthly review の "regime / volatility / intervention" slice (`monthly_review.py`
  の集計) も同じ ensemble 行パターンで Phase A 内で揃える。Phase A の
  P0 stop-loss の達成基準は「weekly + monthly のすべての range_hit 系
  メトリクスが 4× インフレなし」であって、weekly top-level だけでは不十分。
...
- `weekly_slice_metrics.csv` の各 `(slice_dimension, label)` について
  ensemble 行が存在することを assert (regime_label / volatility_label
  / intervention_risk / event_tag 全 dimension)。
- `monthly_review.py` 集計の同等エクスポートも同じ ensemble 行を持つこと。
