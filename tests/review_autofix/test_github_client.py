from ugh_quantamental.review_autofix.github_client import (
    GithubClient,
    GithubEvent,
...
def test_list_review_comments_for_review_filters_by_review_id() -> None:
    client = GithubClient("token")
    client.list_review_comments = lambda repo, pr: (  # type: ignore[method-assign]
        {"id": 1, "pull_request_review_id": 10, "body": "a"},
        {"id": 2, "pull_request_review_id": 20, "body": "b"},
        {"id": 3, "pull_request_review_id": 10, "body": "c"},
    )
    result = client.list_review_comments_for_review("acme/repo", 1, 10)
    assert len(result) == 2
    assert all(c["pull_request_review_id"] == 10 for c in result)


def test_list_review_comments_for_review_returns_empty_tuple_on_no_match() -> None:
    client = GithubClient("token")
    client.list_review_comments = lambda repo, pr: (  # type: ignore[method-assign]
        {"id": 1, "pull_request_review_id": 20, "body": "a"},
    )
    result = client.list_review_comments_for_review("acme/repo", 1, 10)
    assert result == ()


def _base_review_body_context() -> ReviewContext:
    return ReviewContext(
        ...
        line=None,
        start_line=None,
        version_discriminator="vd",
    )


def test_build_context_from_inline_comment_produces_diff_comment_context() -> None:
    base = _base_review_body_context()
    comment = {
        "id": 44,
    ...
    ctx = build_context_from_inline_comment(base, comment)
    assert ctx.kind == ReviewKind.diff_comment
    assert ctx.repository == "acme/repo"
    assert ctx.pr_number == 12
    assert ctx.review_comment_id == 44
    assert ctx.review_id == 10
    assert ctx.head_sha == "sha"
    assert ctx.path == "src/foo.py"
    assert ctx.diff_hunk == "@@ -1,3 +1,3 @@"
    assert ctx.line == 310
    assert ctx.reviewer_login == "alice"
    assert ctx.review_comment_node_id == "PRRC_44"
    assert ctx.same_repo is True


def test_build_context_from_inline_comment_inherits_pr_metadata() -> None:
    base = _base_review_body_context()
    comment = {
        "id": 99,
        "pull_request_review_id": 10,
    ...
    ctx = build_context_from_inline_comment(base, comment)
    assert ctx.base_ref == base.base_ref
    assert ctx.head_ref == base.head_ref
    assert ctx.head_sha == base.head_sha
    assert ctx.same_repo == base.same_repo
