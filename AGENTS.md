...
### 5.5 レビュー指摘の採否判断 (実害基準)

review 指摘は**コード / skill / docs への実害**で採否と対応の重さを決める。
全部直すのはオーバーエンジニアリングであり、逆に全部流すと trap が残る。

| 実害 | 例 | 対応 |
|---|---|---|
| **動作が変わる** | version default の不同期で v2.6 の出力が v2.5 として記録される (#122) | 直す。最優先 |
| **後続作業を誤導する** | 因果の誤りで brief が誤った経路を対象にする (#123 の state→direction) | 直す。誤りの記録も残す |
| **記述の正確性のみ** | 日付の誤り、表記ゆれ | 事実に関わるなら直す。断定できないなら**断定を外す**のが最小の修正 |
| **実害なし** | 好みの問題、既存方針との相違、指摘自体が事実誤認 | **却下してよい**。ただし根拠を実データ/コードで示して返信する |

判断のしかた:

- **まず検証する。** 指摘が正しいかをコード・データで確かめてから採否を決める。
  bot の指摘も人の指摘も、正しいこともあれば外していることもある
  (#122 で「skill は月曜 dir を見るべき」は実データが逆を示したため却下した)。
- **対応の重さを実害に比例させる。** 因果の誤りは節ごと書き直す価値があるが、
  日付 1 つに全面改稿は要らない。
- **却下は説明を伴う。** 実データやコード参照を添えて返信し、resolve する。
  黙ってスルーしない。
- **自分の誤りは記録に残す。** 特に「後続作業を誤導する」級を直したときは、
  何を撤回したかを doc に書く。次に読む人が古い結論を拾わないため。
